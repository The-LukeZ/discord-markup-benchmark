var e = class {
	...
	enqueue(t) {
		let n = new e(t);
		this.#head ? (this.#tail.next = n, this.#tail = n) : (this.#head = n, this.#tail = n), this.#size++;
	}
	dequeue() {
		let e = this.#head;
...
function n(e) {
	...
	let n = new t(), i = 0, a = () => {
		...
		new Promise((e) => {
			n.enqueue(e);
		}).then(s.bind(void 0, t, r, o)), i < e && a();
	}, l = (e, ...t) => new Promise((n) => {
		c(e, n, t);
	});
	return Object.defineProperties(l, {
		...
		concurrency: {
			get: () => e,
			set(t) {
				r(t), e = t, queueMicrotask(() => {
					for (; i < e && n.size > 0;) a();
				});
			}
		},
		map: { async value(e, t) {
			let n = e.map((e, n) => this(t, e, n));
...
const i = Object.freeze({
	...
	infinity: 1.96
}), a = Object.freeze(() => {}), o = (e, t) => {
	let n = new globalThis.Event(e);
	return t && Object.defineProperty(n, "task", {
		configurable: !1,
		enumerable: !1,
		value: t,
		writable: !1
	...
	let n = new globalThis.Event("error");
	return Object.defineProperty(n, "task", {
		configurable: !1,
		enumerable: !1,
		value: e,
		writable: !1
...
let c = /* @__PURE__ */ function(e) {
	return e.browser = "browser", e.bun = "bun", e.deno = "deno", e["edge-light"] = "edge-light", e.fastly = "fastly", e.hermes = "hermes", e.jsc = "jsc", e.lagon = "lagon", e.moddable = "moddable", e.netlify = "netlify", e.node = "node", e["quickjs-ng"] = "quickjs-ng", e.spidermonkey = "spidermonkey", e.v8 = "v8", e.workerd = "workerd", e;
}({});
const l = !!globalThis.Bun || !!globalThis.process?.versions?.bun, u = !!globalThis.Deno, d = globalThis.process?.release?.name === "node", f = !!globalThis.HermesInternal, p = globalThis.navigator?.userAgent === "Cloudflare-Workers", m = !!globalThis.navigator?.userAgent?.toLowerCase?.()?.includes?.("quickjs-ng"), h = typeof globalThis.Netlify == "object", g = typeof globalThis.EdgeRuntime == "string", _ = !!globalThis.__lagon__, v = !!globalThis.fastly, y = !!globalThis.$262 && !!globalThis.lockdown && !!globalThis.AsyncDisposableStack, b = !!globalThis.d8, x = !!globalThis.inIon && !!globalThis.performance?.mozMemory, S = !!globalThis.$ && "IsHTMLDDA" in globalThis.$, C = !!globalThis.window && !!globalThis.navigator, w = (() => l ? c.bun : u ? c.deno : d ? c.node : f ? c.hermes : h ? c.netlify : g ? c["edge-light"] : _ ? c.lagon : v ? c.fastly : p ? c.workerd : m ? c["quickjs-ng"] : y ? c.moddable : b ? c.v8 : x ? c.spidermonkey : S ? c.jsc : C ? c.browser : "unknown")(), T = (() => w === c.bun ? globalThis.Bun?.version : w === c.deno ? globalThis.Deno?.version?.deno : w === c.node ? globalThis.process?.versions?.node : w === c.hermes ? globalThis.HermesInternal?.getRuntimeProperties?.()?.["OSS Release Version"] : w === c.v8 ? globalThis.version?.() : w === c["quickjs-ng"] ? globalThis.navigator?.userAgent?.split?.("/")[1] : "unknown")(), E = (e) => e / 1e6, D = (e) => e * 1e6, O = (e, t, n) => {
	...
	let i = r >= 1 ? Math.floor(Math.log10(r)) + 1 : 0, a = Math.max(0, t - i);
	return a = Math.min(a, n), e.toFixed(a);
};
let k;
k = typeof globalThis.process?.hrtime?.bigint == "function" ? globalThis.process.hrtime.bigint.bind(process.hrtime) : () => {
	throw Error("hrtime.bigint() is not supported in this JS environment");
};
const A = () => E(Number(k())), j = performance.now.bind(performance), M = j, N = (e) => e !== null && (typeof e == "object" || typeof e == "function") && typeof e.then == "function", P = (e) => e?.constructor === (async () => {}).constructor, F = (e) => {
	if (e == null) return !1;
	if (P(e)) return !0;
	try {
		let t = e(), n = N(t);
	...
	if (e.length === 0) throw Error("samples must not be empty");
	return e.reduce((e, t) => e + t, 0) / e.length || 0;
}, L = (e, t = I(e)) => {
	if (e.length <= 1) return 0;
	let n = e.reduce((e, n) => e + (n - t) ** 2, 0);
	return n / (e.length - 1);
}, R = (e, t) => {
	if (e.length === 0) throw Error("samples must not be empty");
	if (t < 0 || t > 1) throw Error("q must be between 0 and 1");
	if (t === 0) return e[0];
	if (t === 1) return e[e.length - 1];
	let n = (e.length - 1) * t, r = Math.floor(n);
	return e[r + 1] == null ? e[r] : e[r] + (n - r) * (e[r + 1] - e[r]);
}, z = (e) => R(e, .5), B = (e) => z([...e].sort((e, t) => e - t)), V = (e, t, n = t(e)) => {
	let r = [];
	for (let t of e) r.push(Math.abs(t - n));
	return t(r);
}, H = (e) => {
	let t = I(e), n = L(e, t), r = Math.sqrt(n), a = r / Math.sqrt(e.length), o = e.length - 1, s = i[(Math.round(o) || 1).toString()] || i.infinity, c = a * s, l = Math.abs(t), u = l === 0 ? Infinity : c / l * 100, d = z(e);
	return {
		aad: V(e, I, t),
		critical: s,
		df: o,
...
var W = class extends EventTarget {
	...
	fnOpts;
	constructor(e, t, n, r = {}) {
		super(), this.bench = e, this.name = t, this.fn = n, this.fnOpts = r, this.async = F(n);
	}
	...
	async run() {
		if (this.result?.error) return this;
		this.dispatchEvent(o("start", this)), await this.bench.opts.setup(this, "run");
		let { error: e, samples: t } = await this.benchmark("run", this.bench.opts.time, this.bench.opts.iterations);
		return await this.bench.opts.teardown(this, "run"), this.processRunResult({
			error: e,
			latencySamples: t
		}), this;
	}
	runSync() {
		if (this.result?.error) return this;
		U(this.bench.concurrency === null, "Cannot use `concurrency` option when using `runSync`"), this.dispatchEvent(o("start", this));
		let e = this.bench.opts.setup(this, "run");
		U(!N(e), "`setup` function must be sync when using `runSync()`");
		let { error: t, samples: n } = this.benchmarkSync("run", this.bench.opts.time, this.bench.opts.iterations), r = this.bench.opts.teardown(this, "run");
		return U(!N(r), "`teardown` function must be sync when using `runSync()`"), this.processRunResult({
			error: t,
			latencySamples: n
		}), this;
	}
	async warmup() {
		...
		this.dispatchEvent(o("warmup", this)), await this.bench.opts.setup(this, "warmup");
		let { error: e } = await this.benchmark("warmup", this.bench.opts.warmupTime, this.bench.opts.warmupIterations);
		await this.bench.opts.teardown(this, "warmup"), this.postWarmup(e);
	}
	warmupSync() {
		if (this.result?.error) return;
		this.dispatchEvent(o("warmup", this));
		let e = this.bench.opts.setup(this, "warmup");
		U(!N(e), "`setup` function must be sync when using `runSync()`");
		let { error: t } = this.benchmarkSync("warmup", this.bench.opts.warmupTime, this.bench.opts.warmupIterations), n = this.bench.opts.teardown(this, "warmup");
		U(!N(n), "`teardown` function must be sync when using `runSync()`"), this.postWarmup(t);
	}
	async benchmark(e, t, r) {
		if (this.fnOpts.beforeAll != null) try {
			await this.fnOpts.beforeAll.call(this, e);
		} catch (e) {
			return { error: e };
		}
		let i = 0, a = [], o = async () => {
			if (!this.bench.opts.signal?.aborted) try {
				this.fnOpts.beforeEach != null && await this.fnOpts.beforeEach.call(this, e);
				let t;
				this.async ? {taskTime: t} = await this.measureOnce() : {taskTime: t} = this.measureOnceSync(), a.push(t), i += t;
			} finally {
				this.fnOpts.afterEach != null && await this.fnOpts.afterEach.call(this, e);
			}
		};
		try {
			let e;
			this.bench.concurrency === "task" && (e = n(Math.max(1, Math.floor(this.bench.threshold))));
			let s = [];
			for (; (i < t || a.length + (e?.activeCount ?? 0) + (e?.pendingCount ?? 0) < r) && !this.bench.opts.signal?.aborted;) this.bench.concurrency === "task" ? s.push(e(o)) : await o();
			!this.bench.opts.signal?.aborted && s.length > 0 ? await Promise.all(s) : s.length > 0 && Promise.allSettled(s);
		} catch (e) {
			return { error: e };
		}
		if (this.fnOpts.afterAll != null) try {
			await this.fnOpts.afterAll.call(this, e);
		} catch (e) {
			return { error: e };
		}
		return { samples: a };
	...
	benchmarkSync(e, t, n) {
		if (this.fnOpts.beforeAll != null) try {
			let t = this.fnOpts.beforeAll.call(this, e);
			U(!N(t), "`beforeAll` function must be sync when using `runSync()`");
		} catch (e) {
			return { error: e };
		}
		let r = 0, i = [], a = () => {
			if (!this.bench.opts.signal?.aborted) try {
				if (this.fnOpts.beforeEach != null) {
					let t = this.fnOpts.beforeEach.call(this, e);
					U(!N(t), "`beforeEach` function must be sync when using `runSync()`");
				}
				let { taskTime: t } = this.measureOnceSync();
				i.push(t), r += t;
			} finally {
				if (this.fnOpts.afterEach != null) {
					let t = this.fnOpts.afterEach.call(this, e);
					U(!N(t), "`afterEach` function must be sync when using `runSync()`");
				}
			}
		};
		try {
			for (; (r < t || i.length < n) && !this.bench.opts.signal?.aborted;) a();
		} catch (e) {
			return { error: e };
		}
		if (this.fnOpts.afterAll != null) try {
			let t = this.fnOpts.afterAll.call(this, e);
			U(!N(t), "`afterAll` function must be sync when using `runSync()`");
		} catch (e) {
			return { error: e };
		}
		return { samples: i };
	...
	async measureOnce() {
		let e = this.bench.opts.now(), t = await this.fn.call(this), n = this.bench.opts.now() - e, r = G(t);
		return r != null && (n = r), {
			fnResult: t,
			taskTime: n
		};
	...
	measureOnceSync() {
		let e = this.bench.opts.now(), t = this.fn.call(this), n = this.bench.opts.now() - e;
		U(!N(t), "task function must be sync when using `runSync()`");
		let r = G(t);
		return r != null && (n = r), {
			fnResult: t,
			taskTime: n
	...
	postWarmup(e) {
		if (e && (this.mergeTaskResult({ error: e }), this.dispatchEvent(s(this, e)), this.bench.dispatchEvent(s(this, e)), this.bench.opts.throws)) throw e;
	}
	processRunResult({ error: e, latencySamples: t }) {
		if (t && t.length > 0) {
			this.runs = t.length;
			let e = t.reduce((e, t) => e + t, 0), n = H(t.sort((e, t) => e - t)), r = t.map((e) => e === 0 ? n.mean === 0 ? 0 : 1e3 / n.mean : 1e3 / e).sort((e, t) => e - t), i = H(r);
			this.mergeTaskResult({
				aborted: this.bench.opts.signal?.aborted ?? !1,
				critical: n.critical,
				df: n.df,
				hz: i.mean,
				latency: n,
		...
		if (e && (this.mergeTaskResult({ error: e }), this.dispatchEvent(s(this, e)), this.bench.dispatchEvent(s(this, e)), this.bench.opts.throws)) throw e;
		this.dispatchEvent(o("cycle", this)), this.bench.dispatchEvent(o("cycle", this)), this.dispatchEvent(o("complete", this));
	}
};
function G(e) {
	if (typeof e == "object" && e && "overriddenDuration" in e && typeof e.overriddenDuration == "number" && Number.isFinite(e.overriddenDuration) && e.overriddenDuration >= 0) return e.overriddenDuration;
}
var K = class extends EventTarget {
	...
	constructor(e = {}) {
		super();
		let { name: t,...n } = e;
		this.name = t, this.runtime = w, this.runtimeVersion = T, this.opts = {
			iterations: 64,
			now: M,
			setup: a,
			teardown: a,
			...
			warmupTime: 250,
			...n
		}, this.opts.signal && this.opts.signal.addEventListener("abort", () => {
			this.dispatchEvent(o("abort"));
		}, { once: !0 });
	}
	add(e, t, n = {}) {
		if (this._tasks.has(e)) throw Error(`Task "${e}" already exists`);
		{
			let r = new W(this, e, t, n);
			this._tasks.set(e, r), this.dispatchEvent(o("add", r));
		}
		return this;
	}
	addEventListener(e, t, n) {
	...
	remove(e) {
		...
		return t && (this.dispatchEvent(o("remove", t)), this._tasks.delete(e)), this;
	}
	removeEventListener(e, t, n) {
		super.removeEventListener(e, t, n);
	}
	reset() {
		this.dispatchEvent(o("reset"));
		for (let e of this._tasks.values()) e.reset();
	}
	async run() {
		this.opts.warmup && await this.warmupTasks();
		let e = [];
		if (this.dispatchEvent(o("start")), this.concurrency === "bench") e = await this.mapTasksConcurrently((e) => e.run());
		else for (let t of this._tasks.values()) e.push(await t.run());
		return this.dispatchEvent(o("complete")), e;
	}
	runSync() {
		U(this.concurrency === null, "Cannot use `concurrency` option when using `runSync`"), this.opts.warmup && this.warmupTasksSync();
		let e = [];
		this.dispatchEvent(o("start"));
		for (let t of this._tasks.values()) e.push(t.runSync());
		return this.dispatchEvent(o("complete")), e;
	}
	table(e) {
		return this.tasks.map((t) => {
			if (t.result) {
				let { error: n, latency: r, throughput: i } = t.result;
				return n ? {
					"Task name": t.name,
					Error: n.message,
					Stack: n.stack
					...
					"Task name": t.name,
					"Latency avg (ns)": `${O(D(r.mean), 5, 2)} \xb1 ${r.rme.toFixed(2)}%`,
					"Latency med (ns)": `${O(D(r.p50), 5, 2)} \xb1 ${O(D(r.mad), 5, 2)}`,
					"Throughput avg (ops/s)": `${Math.round(i.mean).toString()} \xb1 ${i.rme.toFixed(2)}%`,
					"Throughput med (ops/s)": `${Math.round(i.p50).toString()} \xb1 ${Math.round(i.mad).toString()}`,
	...
	async mapTasksConcurrently(e) {
		let t = n(Math.max(1, Math.floor(this.threshold))), r = [];
		for (let n of this._tasks.values()) r.push(t(() => e(n)));
		return Promise.all(r);
	}
	async warmupTasks() {
		if (this.dispatchEvent(o("warmup")), this.concurrency === "bench") await this.mapTasksConcurrently((e) => e.warmup());
		else for (let e of this._tasks.values()) await e.warmup();
	}
	warmupTasksSync() {
		this.dispatchEvent(o("warmup"));
		for (let e of this._tasks.values()) e.warmupSync();
	}
};
export { K as Bench, W as Task, A as hrtimeNow, E as nToMs, M as now };
